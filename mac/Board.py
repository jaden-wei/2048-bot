...
class Board:
    ...
    def getNewBoard(self):
        ...
        self.boardArr[0][0] = self.getTileVal(
            boardscreenshot.getpixel((50, 50)))
        self.boardArr[0][1] = self.getTileVal(
            boardscreenshot.getpixel((boardscreenshot.width / 4 + 50, 50)))
        self.boardArr[0][2] = self.getTileVal(
            boardscreenshot.getpixel((boardscreenshot.width / 2 + 50, 50)))
        self.boardArr[0][3] = self.getTileVal(
            boardscreenshot.getpixel((boardscreenshot.width * (3/4) + 50, 50)))

        self.boardArr[1][0] = self.getTileVal(
            boardscreenshot.getpixel((50, boardscreenshot.height / 4 + 50)))
        self.boardArr[1][1] = self.getTileVal(boardscreenshot.getpixel(
            (boardscreenshot.width / 4 + 50, boardscreenshot.height / 4 + 50)))
        self.boardArr[1][2] = self.getTileVal(boardscreenshot.getpixel(
            (boardscreenshot.width / 2 + 50, boardscreenshot.height / 4 + 50)))
        self.boardArr[1][3] = self.getTileVal(boardscreenshot.getpixel(
            (boardscreenshot.width * (3/4) + 50, boardscreenshot.height / 4 + 50)))

        self.boardArr[2][0] = self.getTileVal(
            boardscreenshot.getpixel((50, boardscreenshot.height / 2 + 50)))
        self.boardArr[2][1] = self.getTileVal(
            boardscreenshot.getpixel((boardscreenshot.width / 4 + 50, boardscreenshot.height / 2 + 50)))
        self.boardArr[2][2] = self.getTileVal(
            boardscreenshot.getpixel((boardscreenshot.width / 2 + 50, boardscreenshot.height / 2 + 50)))
        self.boardArr[2][3] = self.getTileVal(
            boardscreenshot.getpixel((boardscreenshot.width * (3/4) + 50, boardscreenshot.height / 2 + 50)))

        self.boardArr[3][0] = self.getTileVal(
            boardscreenshot.getpixel((50, boardscreenshot.height * (3/4) + 50)))
        self.boardArr[3][1] = self.getTileVal(
            boardscreenshot.getpixel((boardscreenshot.width / 4 + 50, boardscreenshot.height * (3/4) + 50)))
        self.boardArr[3][2] = self.getTileVal(
            boardscreenshot.getpixel((boardscreenshot.width / 2 + 50, boardscreenshot.height * (3/4) + 50)))
        self.boardArr[3][3] = self.getTileVal(
            boardscreenshot.getpixel((boardscreenshot.width * (3/4) + 50, boardscreenshot.height * (3/4) + 50)))

        # for i in range(4):
        #     for j in range(4):
        #         print(self.boardArr[i][j], end=" ")
        #     print()

    def getTileVal(self, rgb_val):
        if self.isCloseTo(rgb_val, (200, 193, 181)):
            return 0
        elif self.isCloseTo(rgb_val, (235, 228, 219)):
            return 2
        elif self.isCloseTo(rgb_val, (237, 225, 203)):
            return 4
        elif self.isCloseTo(rgb_val, (225, 183, 138)):
            return 8
        elif self.isCloseTo(rgb_val, (232, 154, 108)):
            return 16
        elif self.isCloseTo(rgb_val, (231, 131, 103)):
            return 32
        elif self.isCloseTo(rgb_val, (229, 104, 72)):
            return 64
        elif self.isCloseTo(rgb_val, (232, 209, 129)):
            return 128
        elif self.isCloseTo(rgb_val, (232, 205, 116)):
            return 256
        elif self.isCloseTo(rgb_val, (231, 201, 102)):
            return 512
        elif self.isCloseTo(rgb_val, (230, 198, 89)):
            return 1024
        elif self.isCloseTo(rgb_val, (231, 196, 80)):
            return 2048
    ...
    def isCloseTo(self, rgb_val, target_val):
        if rgb_val[0] < target_val[0] - 10 or rgb_val[0] > target_val[0] + 10:
            return False
        if rgb_val[1] < target_val[1] - 10 or rgb_val[1] > target_val[1] + 10:
            return False
        if rgb_val[2] < target_val[2] - 10 or rgb_val[2] > target_val[2] + 10:
            return False
        return True
